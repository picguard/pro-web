((a,b,c)=>{a[b]=a[b]||{}
a[b][c]=a[b][c]||[]
a[b][c].push({p:"main.dart.js_1",e:"beginPart"})})(self,"$__dart_deferred_initializers__","eventLog")
$__dart_deferred_initializers__.current=function(a,b,c,$){var B,C,A={
bj9(d,e,f){var w,v,u=null,t=new B.lb(C.pP,x.c),s=new B.lb(C.h5,x.c)
s=new A.a6L(t,s)
s.JP(d,u,e,u)
w=s.a
v=t.r
v.toString
w.r=v
t.r=A.bEs(s)
return s},
bEp(d,e){switch(e){case"appName":return new A.b46()
case"homePage.appPreview":return y.p
case"homePage.imageDescription":return"Si prega di caricare le immagini."
case"homePage.description":return y.M
case"homePage.textLabel":return"Testo"
case"homePage.textLabelDescription":return y._
case"homePage.textValidator":return"Inserisci il testo."
case"homePage.textInput":return y.N
case"homePage.colorLabel":return"Colore"
case"homePage.colorValidator":return"Seleziona un colore."
case"homePage.opacityLabel":return"Opacit\xe0"
case"homePage.fontLabel":return"Font"
case"homePage.fontLabelDescription":return y.b
case"homePage.fontValidator":return"Seleziona un font."
case"homePage.fontSizeLabel":return"Dimensione del font"
case"homePage.textGapLabel":return"Spaziatura del testo"
case"homePage.textGapDescription":return y.f
case"homePage.rowGapLabel":return"Interlinea"
case"homePage.rowGapDescription":return"La spaziatura tra ogni riga di testo"
case"homePage.limitValidator":return new A.b47()
case"homePage.formatValidator":return"Supportare solo i formati PNG e JPEG"
case"homePage.preview":return"Anteprima"
case"homePage.save":return"Salva"
case"homePage.savedSuccess":return"Salvataggio riuscito"
case"homePage.savedFailure":return"Impossibile salvare l'immagine(i)"
case"homePage.saveInfo":return new A.b48()
case"aboutPage.version":return new A.b49()
case"aboutPage.copyright":return new A.b4a()
case"aboutPage.slogan":return"Le tue foto, la tua firma."
case"aboutPage.readme":return"Introduzione del progetto"
case"aboutPage.appLicense":return"Licenza di applicazione"
case"aboutPage.changelog":return"Changelog"
case"aboutPage.thirdPartyLicense":return"Licenze di terze parti"
case"colors.white":return"Bianco"
case"colors.grey":return"Grigio"
case"colors.black":return"Nero"
case"colors.red":return"Rosso"
case"colors.orange":return"Arancione"
case"colors.blue":return"Blu"
case"menus.about":return new A.b4c()
case"menus.hide":return new A.b4d()
case"menus.hideOthers":return"Nascondere gli altri"
case"menus.showAll":return"Mostra tutto"
case"menus.exit":return new A.b4e()
case"menus.help":return"Aiuto"
case"menus.support":return"Supporto"
case"menus.userAgreement":return"Accordo utente"
case"menus.privacy":return"Politica sulla riservatezza"
case"buttons.agree":return"Accetto"
case"buttons.cancel":return"Annulla"
case"buttons.ignore":return"Ignora"
case"buttons.turnOn":return"Accendi"
case"dialogs.selectColor":return"Seleziona un colore"
case"dialogs.selectFont":return"Seleziona un carattere"
case"dialogs.exitConfirm.exit":return"Uscita"
case"dialogs.exitConfirm.cancel":return"Annulla"
case"dialogs.exitConfirm.title":return"Chiudi conferma"
case"dialogs.exitConfirm.description":return"Sei sicuro di voler chiudere questa finestra?"
case"dialogs.permissions.photos.title":return"Consenti l'accesso al tuo album"
case"dialogs.permissions.photos.description":return new A.b4f()
case"dialogs.permissions.storage.title":return"Consenti l'accesso al tuo archivio"
case"dialogs.permissions.storage.description":return new A.b4g()
case"dialogs.licenseDialog.licenseDialogTitle":return"Termini e Condizioni"
case"dialogs.licenseDialog.licenseDialogContentContent":return new A.b4h()
case"dialogs.licenseDialog.licenseDialogContentTip":return y.n
case"dialogs.licenseDialog.licenseDialogContentPrefix":return y.e
case"dialogs.licenseDialog.licenseDialogContentUserAgreement":return"Contratto d'uso"
case"dialogs.licenseDialog.licenseDialogContentAnd":return" e "
case"dialogs.licenseDialog.licenseDialogContentPrivacyPolicy":return"L'accordo sulla privacy"
case"dialogs.licenseDialog.licenseDialogContentSuffix":return y.k
case"dialogs.licenseDialog.iosPermissions.0":return new A.b4i()
case"dialogs.licenseDialog.androidPermissions.0":return new A.b4j()
case"dialogs.licenseDialog.androidPermissions.1":return new A.b4k()
case"dialogs.licenseDialog.androidPermissions.2":return new A.b4l()
case"dialogs.licenseDialog.androidPermissions.3":return new A.b4n()
case"dialogs.settingsDialog.settings":return"Impostazioni"
case"dialogs.settingsDialog.languages":return"Lingue"
case"dialogs.settingsDialog.themes.manual":return"Modalit\xe0 manuale"
case"dialogs.settingsDialog.themes.system":return"Modalit\xe0 automatica"
case"dialogs.settingsDialog.themes.dark":return"Modalit\xe0 scura"
case"dialogs.settingsDialog.themes.light":return"Modalit\xe0 chiara"
case"dialogs.settingsDialog.themesDescription":return y.c
case"locales.en":return"English"
case"locales.it":return"Italian"
case"locales.zh":return"\u4e2d\u6587"
default:return null}},
bEs(d){return new A.b4w(d)},
a6L:function a6L(d,e){var _=this
_.x=d
_.ay=_.ax=_.at=_.as=_.Q=_.z=_.y=$
_.a=e
_.w=_.r=_.f=_.e=_.d=_.c=_.b=$},
b2U:function b2U(){},
b2u:function b2u(){},
b2y:function b2y(){},
b2W:function b2W(){},
b2w:function b2w(){},
b2A:function b2A(d,e){var _=this
_.f=d
_.y=_.x=_.w=$
_.a=e
_.e=_.d=_.c=$},
b2N:function b2N(d,e){var _=this
_.d=d
_.f=_.e=$
_.a=e
_.c=_.b=$},
b2B:function b2B(){},
b2G:function b2G(){},
b2C:function b2C(){},
b2D:function b2D(){},
b2E:function b2E(){},
b2F:function b2F(){},
b2P:function b2P(d,e){var _=this
_.c=d
_.d=$
_.a=e
_.b=$},
aj7:function aj7(){},
aj9:function aj9(){},
b2Q:function b2Q(){},
b4w:function b4w(d){this.a=d},
b46:function b46(){},
b47:function b47(){},
b48:function b48(){},
b49:function b49(){},
b4a:function b4a(){},
b4c:function b4c(){},
b4d:function b4d(){},
b4e:function b4e(){},
b4f:function b4f(){},
b4g:function b4g(){},
b4h:function b4h(){},
b4i:function b4i(){},
b4j:function b4j(){},
b4k:function b4k(){},
b4l:function b4l(){},
b4n:function b4n(){}}
B=c[0]
C=c[2]
A=a.updateHolder(c[3],A)
A.a6L.prototype={
i(d,e){var w=this.x.ww(e)
return w==null?this.a.ww(e):w},
gu6(){var w=this.y
if(w===$){w!==$&&B.a5()
w=this.y=this}return w},
lT(d,e){switch(e.a){case 0:return"PicGuard"
case 1:return"PicGuard Pro"}},
geh(){var w=this,v=w.z
if(v===$){w.gu6()
w.z!==$&&B.a5()
v=w.z=new A.b2U()}return v},
gnH(){var w=this,v=w.Q
if(v===$){w.gu6()
w.Q!==$&&B.a5()
v=w.Q=new A.b2u()}return v},
gph(){var w=this,v=w.as
if(v===$){w.gu6()
w.as!==$&&B.a5()
v=w.as=new A.b2y()}return v},
geU(){var w=this,v=w.at
if(v===$){w.gu6()
w.at!==$&&B.a5()
v=w.at=new A.b2W()}return v},
gdP(d){var w=this,v=w.ax
if(v===$){w.gu6()
w.ax!==$&&B.a5()
v=w.ax=new A.b2w()}return v},
gdK(){var w,v=this,u=v.ay
if(u===$){w=v.gu6()
v.ay!==$&&B.a5()
u=v.ay=new A.b2A(w,w)}return u},
gR6(){var w=x.g
return B.a6(["en","English","it","Italian","zh","\u4e2d\u6587"],w,w)},
gta(){return this.x}}
A.b2U.prototype={
gO1(){return y.p},
gkm(d){return y.M},
gSh(){return"Testo"},
gSi(){return y._},
gSj(){return"Inserisci il testo."},
gSg(){return y.N},
gOC(){return"Colore"},
gOD(){return"Seleziona un colore."},
gRx(){return"Opacit\xe0"},
gQ3(){return"Font"},
gQ4(){return y.b},
gQ6(){return"Seleziona un font."},
gQ5(){return"Dimensione del font"},
gSf(){return"Spaziatura del testo"},
gSe(){return y.f},
gSc(){return"Interlinea"},
gSb(){return"La spaziatura tra ogni riga di testo"},
GO(d,e){return"Supporta fino a "+e+" immagini, puoi anche caricare "+d+" immagini."},
gG9(){return"Supportare solo i formati PNG e JPEG"},
gRO(){return"Anteprima"},
gIQ(d){return"Salva"},
gIT(){return"Salvataggio riuscito"},
gBx(){return"Impossibile salvare l'immagine(i)"},
IR(d,e){return"Sono state salvate "+e+y.I+d+" l'immagine(i)."}}
A.b2u.prototype={
SP(d,e,f){return"Versione "+f+"("+e+")"},
OY(d,e){return"Copyright \xa9 "+e+" "+d+". Tutti i diritti riservati."},
gJi(){return"Le tue foto, la tua firma."},
gRW(){return"Introduzione del progetto"},
gO0(){return"Licenza di applicazione"},
gOq(){return"Changelog"},
gSo(){return"Licenze di terze parti"}}
A.b2y.prototype={
gT_(){return"Bianco"},
gIO(){return"Grigio"},
gOd(){return"Nero"},
grW(){return"Rosso"},
gRy(){return"Arancione"},
gqT(){return"Blu"}}
A.b2W.prototype={
uD(d){return"Di "+d},
Qv(d){return"Nascondere "+d},
gQw(){return"Nascondere gli altri"},
gJf(){return"Mostra tutto"},
FP(d){return"Esentato "+d},
gzY(){return"Aiuto"},
gtM(){return"Supporto"},
gwd(){return"Accordo utente"},
gvY(){return"Politica sulla riservatezza"}}
A.b2w.prototype={
gNX(){return"Accetto"},
gOj(d){return"Annulla"},
gGy(){return"Ignora"},
gIi(){return"Accendi"}}
A.b2A.prototype={
gIZ(){return"Seleziona un colore"},
gJ_(){return"Seleziona un carattere"},
gn1(d){var w,v=this.w
if(v===$){w=this.f
v!==$&&B.a5()
v=this.w=new A.b2N(w,w)}return v},
glk(){var w=this.x
if(w===$){w!==$&&B.a5()
w=this.x=new A.b2B()}return w},
gk7(){var w,v=this.y
if(v===$){w=this.f
v!==$&&B.a5()
v=this.y=new A.b2P(w,w)}return v}}
A.b2N.prototype={
gvX(){var w=this.e
if(w===$){w!==$&&B.a5()
w=this.e=new A.aj7()}return w},
gtC(d){var w=this.f
if(w===$){w!==$&&B.a5()
w=this.f=new A.aj9()}return w}}
A.b2B.prototype={
gR1(){return"Termini e Condizioni"},
QW(d){return y.d+d+"."},
gR_(){return y.n},
gQX(){return y.e},
gR0(){return"Contratto d'uso"},
gQV(){return" e "},
gQY(){return"L'accordo sulla privacy"},
gQZ(){return y.k},
gQH(){return[new A.b2G()]},
gNY(){return[new A.b2C(),new A.b2D(),new A.b2E(),new A.b2F()]}}
A.b2P.prototype={
gtt(){return"Impostazioni"},
gQP(d){return"Lingue"},
gt4(){var w=this.d
if(w===$){w!==$&&B.a5()
w=this.d=new A.b2Q()}return w},
gSm(){return y.c}}
A.aj7.prototype={
gfk(d){return"Consenti l'accesso al tuo album"},
m_(d,e){return y.B+B.k(e)+y.S}}
A.aj9.prototype={
gfk(d){return"Consenti l'accesso al tuo archivio"},
m_(d,e){return y.g+B.k(e)+y.j}}
A.b2Q.prototype={
gRd(){return"Modalit\xe0 manuale"},
gJN(){return"Modalit\xe0 automatica"},
gFt(){return"Modalit\xe0 scura"},
gR2(){return"Modalit\xe0 chiara"}}
var z=a.updateTypes(["i({appName!E})"])
A.b2G.prototype={
$1$appName(d){return y.C+B.k(d)+y.Q},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.r}},
$S:4}
A.b2C.prototype={
$1$appName(d){return y.a+B.k(d)+y.l},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.r}},
$S:4}
A.b2D.prototype={
$1$appName(d){return"Per filigranare le tue immagini, "+B.k(d)+y.E},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.r}},
$S:4}
A.b2E.prototype={
$1$appName(d){return"Per poter salvare le immagini, "+B.k(d)+y.u},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.r}},
$S:4}
A.b2F.prototype={
$1$appName(d){return y.C+B.k(d)+y.T},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.r}},
$S:4}
A.b4w.prototype={
$1(d){return A.bEp(this.a,d)},
$S:80}
A.b46.prototype={
$1$flavor(d){switch(d.a){case 0:return"PicGuard"
case 1:return"PicGuard Pro"}},
$C:"$1$flavor",
$R:0,
$D(){return{flavor:C.r}},
$S:126}
A.b47.prototype={
$2$lastImages$maxImages(d,e){return"Supporta fino a "+B.k(e)+" immagini, puoi anche caricare "+B.k(d)+" immagini."},
$C:"$2$lastImages$maxImages",
$R:0,
$D(){return{lastImages:C.r,maxImages:C.r}},
$S:174}
A.b48.prototype={
$2$failedNum$succeedNum(d,e){return"Sono state salvate "+B.k(e)+y.I+B.k(d)+" l'immagine(i)."},
$C:"$2$failedNum$succeedNum",
$R:0,
$D(){return{failedNum:C.r,succeedNum:C.r}},
$S:142}
A.b49.prototype={
$2$buildNumber$version(d,e){return"Versione "+B.k(e)+"("+B.k(d)+")"},
$C:"$2$buildNumber$version",
$R:0,
$D(){return{buildNumber:C.r,version:C.r}},
$S:154}
A.b4a.prototype={
$2$appName$year(d,e){return"Copyright \xa9 "+B.k(e)+" "+B.k(d)+". Tutti i diritti riservati."},
$C:"$2$appName$year",
$R:0,
$D(){return{appName:C.r,year:C.r}},
$S:156}
A.b4c.prototype={
$1$appName(d){return"Di "+B.k(d)},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.r}},
$S:4}
A.b4d.prototype={
$1$appName(d){return"Nascondere "+B.k(d)},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.r}},
$S:4}
A.b4e.prototype={
$1$appName(d){return"Esentato "+B.k(d)},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.r}},
$S:4}
A.b4f.prototype={
$1$appName(d){return y.B+B.k(d)+y.S},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.r}},
$S:4}
A.b4g.prototype={
$1$appName(d){return y.g+B.k(d)+y.j},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.r}},
$S:4}
A.b4h.prototype={
$1$appName(d){return y.d+B.k(d)+"."},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.r}},
$S:4}
A.b4i.prototype={
$1$appName(d){return y.C+B.k(d)+y.Q},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.r}},
$S:4}
A.b4j.prototype={
$1$appName(d){return y.a+B.k(d)+y.l},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.r}},
$S:4}
A.b4k.prototype={
$1$appName(d){return"Per filigranare le tue immagini, "+B.k(d)+y.E},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.r}},
$S:4}
A.b4l.prototype={
$1$appName(d){return"Per poter salvare le immagini, "+B.k(d)+y.u},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.r}},
$S:4}
A.b4n.prototype={
$1$appName(d){return y.C+B.k(d)+y.T},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.r}},
$S:4};(function installTearOffs(){var w=a.installInstanceTearOff
w(A.aj7.prototype,"gkm",1,0,null,["$1$appName"],["m_"],0,0,0)
w(A.aj9.prototype,"gkm",1,0,null,["$1$appName"],["m_"],0,0,0)})();(function inheritance(){var w=a.inherit,v=a.inheritMany
w(A.a6L,B.fZ)
w(A.b2U,B.MW)
w(A.b2u,B.MO)
w(A.b2y,B.MQ)
w(A.b2W,B.MX)
w(A.b2w,B.MP)
w(A.b2A,B.MR)
w(A.b2N,B.MT)
w(A.b2B,B.MS)
v(B.mN,[A.b2G,A.b2C,A.b2D,A.b2E,A.b2F,A.b4w,A.b46,A.b47,A.b48,A.b49,A.b4a,A.b4c,A.b4d,A.b4e,A.b4f,A.b4g,A.b4h,A.b4i,A.b4j,A.b4k,A.b4l,A.b4n])
w(A.b2P,B.MU)
w(A.aj7,B.BU)
w(A.aj9,B.BV)
w(A.b2Q,B.MV)})()
B.bb7(b.typeUniverse,JSON.parse('{"a6L":{"fZ":[],"jE":["fn","fZ"]}}'))
var y={E:" deve accedere al tuo spazio di archiviazione. (android.permission.READ_EXTERNAL_STORAGE)",u:" deve accedere al tuo spazio di archiviazione. (android.permission.WRITE_EXTERNAL_STORAGE)",Q:" deve accedere alle tue foto. (NSPhotoLibraryUsageDescription)",T:" deve accedere alle tue foto. (android.permission.READ_MEDIA_IMAGES)",l:" deve avere accesso a Internet. (android.permission.INTERNET)",S:" l'autorizzazione a visitare il tuo album.",j:" l'autorizzazione ad accedere al tuo archivio.",I:" l'immagine(i) e non \xe8 stato possibile salvare ",k:", inizia a utilizzare i nostri servizi dopo aver accettato tutti i termini.",p:"ATTENZIONE: Versione di sviluppo, basata automaticamente su ogni commit",g:"Accedi alle Impostazioni del tuo telefono per concedere a ",b:"Alcune lingue non supportano i font personalizzati",n:"Di seguito \xe8 riportato l'elenco delle autorizzazioni richieste da questa APP:",c:"Dopo averlo attivato, la modalit\xe0 scura verr\xe0 attivata o disattivata in base al sistema",_:"Il testo verr\xe0 aggiunto all'immagine come filigrana",d:"La protezione della privacy e delle informazioni personali degli utenti \xe8 un principio fondamentale di ",f:"La spaziatura tra il testo in ogni riga di testo",C:"Per aggiungere una filigrana alle tue immagini, ",a:"Per poter utilizzare il servizio Sentry, ",e:"Prima di utilizzare i servizi di questa APP, ti preghiamo di leggere attentamente e accettare il ",M:"Questa app non caricher\xe0 alcuna immagine sul server, tutte le operazioni vengono completate localmente",N:"Questo certificato \xe8 utilizzato solo per la gestione di xx attivit\xe0 e non \xe8 valido per altri scopi.",B:"Vai alle Impostazioni del tuo telefono per concedere a "}
var x={g:B.al("i"),c:B.al("lb<fn,fZ>")}};
((a,b)=>{a[b]=a.current
a.eventLog.push({p:"main.dart.js_1",e:"endPart",h:b})})($__dart_deferred_initializers__,"5AjVWyXZFb2Dvs9mzPKHQ8MbZ4w=");