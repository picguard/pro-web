((a,b,c)=>{a[b]=a[b]||{}
a[b][c]=a[b][c]||[]
a[b][c].push({p:"main.dart.js_1",e:"beginPart"})})(self,"$__dart_deferred_initializers__","eventLog")
$__dart_deferred_initializers__.current=function(a,b,c,$){var B,C,A={
bif(d,e,f){var w,v,u=null,t=new B.kZ(C.po,x.c),s=new B.kZ(C.fZ,x.c)
s=new A.a6a(t,s)
s.K1(d,u,e,u)
w=s.a
v=t.r
v.toString
w.r=v
t.r=A.bDn(s)
return s},
bDk(d,e){switch(e){case"appName":return new A.b2G()
case"homePage.appPreview":return y.p
case"homePage.imageDescription":return"Si prega di caricare le immagini."
case"homePage.description":return y.M
case"homePage.textLabel":return"Testo"
case"homePage.textLabelDescription":return y._
case"homePage.textValidator":return"Inserisci il testo."
case"homePage.textInput":return y.N
case"homePage.colorLabel":return"Colore"
case"homePage.colorValidator":return"Seleziona un colore."
case"homePage.opacityLabel":return"Opacit\xe0"
case"homePage.fontLabel":return"Font"
case"homePage.fontLabelDescription":return y.b
case"homePage.fontValidator":return"Seleziona un font."
case"homePage.fontSizeLabel":return"Dimensione del font"
case"homePage.textGapLabel":return"Spaziatura del testo"
case"homePage.textGapDescription":return y.f
case"homePage.rowGapLabel":return"Interlinea"
case"homePage.rowGapDescription":return"La spaziatura tra ogni riga di testo"
case"homePage.limitValidator":return new A.b2H()
case"homePage.formatValidator":return"Supportare solo i formati PNG e JPEG"
case"homePage.preview":return"Anteprima"
case"homePage.save":return"Salva"
case"homePage.savedSuccess":return"Salvataggio riuscito"
case"homePage.savedFailure":return"Impossibile salvare l'immagine(i)"
case"homePage.saveInfo":return new A.b2I()
case"aboutPage.version":return new A.b2J()
case"aboutPage.copyright":return new A.b2K()
case"aboutPage.slogan":return"Le tue foto, la tua firma."
case"aboutPage.readme":return"Introduzione del progetto"
case"aboutPage.appLicense":return"Licenza di applicazione"
case"aboutPage.changelog":return"Changelog"
case"aboutPage.thirdPartyLicense":return"Licenze di terze parti"
case"colors.white":return"Bianco"
case"colors.grey":return"Grigio"
case"colors.black":return"Nero"
case"colors.red":return"Rosso"
case"colors.orange":return"Arancione"
case"colors.blue":return"Blu"
case"menus.about":return new A.b2M()
case"menus.hide":return new A.b2N()
case"menus.hideOthers":return"Nascondere gli altri"
case"menus.showAll":return"Mostra tutto"
case"menus.exit":return new A.b2O()
case"menus.help":return"Aiuto"
case"menus.support":return"Supporto"
case"menus.userAgreement":return"Accordo utente"
case"menus.privacy":return"Politica sulla riservatezza"
case"buttons.agree":return"Accetto"
case"buttons.cancel":return"Annulla"
case"buttons.ignore":return"Ignora"
case"buttons.turnOn":return"Accendi"
case"dialogs.selectColor":return"Seleziona un colore"
case"dialogs.selectFont":return"Seleziona un carattere"
case"dialogs.exitConfirm.exit":return"Uscita"
case"dialogs.exitConfirm.cancel":return"Annulla"
case"dialogs.exitConfirm.title":return"Chiudi conferma"
case"dialogs.exitConfirm.description":return"Sei sicuro di voler chiudere questa finestra?"
case"dialogs.permissions.photos.title":return"Consenti l'accesso al tuo album"
case"dialogs.permissions.photos.description":return new A.b2P()
case"dialogs.permissions.storage.title":return"Consenti l'accesso al tuo archivio"
case"dialogs.permissions.storage.description":return new A.b2Q()
case"dialogs.licenseDialog.licenseDialogTitle":return"Termini e Condizioni"
case"dialogs.licenseDialog.licenseDialogContentContent":return new A.b2R()
case"dialogs.licenseDialog.licenseDialogContentTip":return y.n
case"dialogs.licenseDialog.licenseDialogContentPrefix":return y.e
case"dialogs.licenseDialog.licenseDialogContentUserAgreement":return"Contratto d'uso"
case"dialogs.licenseDialog.licenseDialogContentAnd":return" e "
case"dialogs.licenseDialog.licenseDialogContentPrivacyPolicy":return"L'accordo sulla privacy"
case"dialogs.licenseDialog.licenseDialogContentSuffix":return y.k
case"dialogs.licenseDialog.iosPermissions.0":return new A.b2S()
case"dialogs.licenseDialog.androidPermissions.0":return new A.b2T()
case"dialogs.licenseDialog.androidPermissions.1":return new A.b2U()
case"dialogs.licenseDialog.androidPermissions.2":return new A.b2V()
case"dialogs.licenseDialog.androidPermissions.3":return new A.b2X()
case"dialogs.settingsDialog.settings":return"Impostazioni"
case"dialogs.settingsDialog.languages":return"Lingue"
case"dialogs.settingsDialog.themes.manual":return"Modalit\xe0 manuale"
case"dialogs.settingsDialog.themes.system":return"Modalit\xe0 automatica"
case"dialogs.settingsDialog.themes.dark":return"Modalit\xe0 scura"
case"dialogs.settingsDialog.themes.light":return"Modalit\xe0 chiara"
case"dialogs.settingsDialog.themesDescription":return y.c
case"locales.en":return"English"
case"locales.it":return"Italian"
case"locales.zh":return"\u4e2d\u6587"
default:return null}},
bDn(d){return new A.b35(d)},
a6a:function a6a(d,e){var _=this
_.x=d
_.ay=_.ax=_.at=_.as=_.Q=_.z=_.y=$
_.a=e
_.w=_.r=_.f=_.e=_.d=_.c=_.b=$},
b1t:function b1t(){},
b13:function b13(){},
b17:function b17(){},
b1v:function b1v(){},
b15:function b15(){},
b19:function b19(d,e){var _=this
_.f=d
_.y=_.x=_.w=$
_.a=e
_.e=_.d=_.c=$},
b1m:function b1m(d,e){var _=this
_.d=d
_.f=_.e=$
_.a=e
_.c=_.b=$},
b1a:function b1a(){},
b1f:function b1f(){},
b1b:function b1b(){},
b1c:function b1c(){},
b1d:function b1d(){},
b1e:function b1e(){},
b1o:function b1o(d,e){var _=this
_.c=d
_.d=$
_.a=e
_.b=$},
aiD:function aiD(){},
aiF:function aiF(){},
b1p:function b1p(){},
b35:function b35(d){this.a=d},
b2G:function b2G(){},
b2H:function b2H(){},
b2I:function b2I(){},
b2J:function b2J(){},
b2K:function b2K(){},
b2M:function b2M(){},
b2N:function b2N(){},
b2O:function b2O(){},
b2P:function b2P(){},
b2Q:function b2Q(){},
b2R:function b2R(){},
b2S:function b2S(){},
b2T:function b2T(){},
b2U:function b2U(){},
b2V:function b2V(){},
b2X:function b2X(){}}
B=c[0]
C=c[2]
A=a.updateHolder(c[3],A)
A.a6a.prototype={
i(d,e){var w=this.x.wL(e)
return w==null?this.a.wL(e):w},
gun(){var w=this.y
if(w===$){w!==$&&B.ab()
w=this.y=this}return w},
m6(d,e){switch(e.a){case 0:return"PicGuard"
case 1:return"PicGuard Pro"}},
gej(){var w=this,v=w.z
if(v===$){w.gun()
w.z!==$&&B.ab()
v=w.z=new A.b1t()}return v},
gnQ(){var w=this,v=w.Q
if(v===$){w.gun()
w.Q!==$&&B.ab()
v=w.Q=new A.b13()}return v},
gpr(){var w=this,v=w.as
if(v===$){w.gun()
w.as!==$&&B.ab()
v=w.as=new A.b17()}return v},
geS(){var w=this,v=w.at
if(v===$){w.gun()
w.at!==$&&B.ab()
v=w.at=new A.b1v()}return v},
gdX(d){var w=this,v=w.ax
if(v===$){w.gun()
w.ax!==$&&B.ab()
v=w.ax=new A.b15()}return v},
gdJ(){var w,v=this,u=v.ay
if(u===$){w=v.gun()
v.ay!==$&&B.ab()
u=v.ay=new A.b19(w,w)}return u},
gR4(){var w=x.g
return B.a6(["en","English","it","Italian","zh","\u4e2d\u6587"],w,w)},
gts(){return this.x}}
A.b1t.prototype={
gO7(){return y.p},
gkp(d){return y.M},
gSg(){return"Testo"},
gSh(){return y._},
gSi(){return"Inserisci il testo."},
gSf(){return y.N},
gOF(){return"Colore"},
gOG(){return"Seleziona un colore."},
gRu(){return"Opacit\xe0"},
gQ5(){return"Font"},
gQ6(){return y.b},
gQ8(){return"Seleziona un font."},
gQ7(){return"Dimensione del font"},
gSe(){return"Spaziatura del testo"},
gSd(){return y.f},
gSa(){return"Interlinea"},
gS9(){return"La spaziatura tra ogni riga di testo"},
H9(d,e){return"Supporta fino a "+e+" immagini, puoi anche caricare "+d+" immagini."},
gGv(){return"Supportare solo i formati PNG e JPEG"},
gRL(){return"Anteprima"},
gJ5(d){return"Salva"},
gJ8(){return"Salvataggio riuscito"},
gBM(){return"Impossibile salvare l'immagine(i)"},
J6(d,e){return"Sono state salvate "+e+y.I+d+" l'immagine(i)."}}
A.b13.prototype={
SM(d,e,f){return"Versione "+f+"("+e+")"},
P0(d,e){return"Copyright \xa9 "+e+" "+d+". Tutti i diritti riservati."},
gJw(){return"Le tue foto, la tua firma."},
gRT(){return"Introduzione del progetto"},
gO6(){return"Licenza di applicazione"},
gOw(){return"Changelog"},
gSn(){return"Licenze di terze parti"}}
A.b17.prototype={
gSX(){return"Bianco"},
gJ3(){return"Grigio"},
gOj(){return"Nero"},
gtd(){return"Rosso"},
gRv(){return"Arancione"},
gra(){return"Blu"}}
A.b1v.prototype={
uQ(d){return"Di "+d},
Qv(d){return"Nascondere "+d},
gQw(){return"Nascondere gli altri"},
gJu(){return"Mostra tutto"},
G9(d){return"Esentato "+d},
gAb(){return"Aiuto"},
gu0(){return"Supporto"},
gwr(){return"Accordo utente"},
gwc(){return"Politica sulla riservatezza"}}
A.b15.prototype={
gO2(){return"Accetto"},
gOp(d){return"Annulla"},
gGU(){return"Ignora"},
gIz(){return"Accendi"}}
A.b19.prototype={
gJd(){return"Seleziona un colore"},
gJe(){return"Seleziona un carattere"},
gn7(d){var w,v=this.w
if(v===$){w=this.f
v!==$&&B.ab()
v=this.w=new A.b1m(w,w)}return v},
glA(){var w=this.x
if(w===$){w!==$&&B.ab()
w=this.x=new A.b1a()}return w},
gk8(){var w,v=this.y
if(v===$){w=this.f
v!==$&&B.ab()
v=this.y=new A.b1o(w,w)}return v}}
A.b1m.prototype={
gwb(){var w=this.e
if(w===$){w!==$&&B.ab()
w=this.e=new A.aiD()}return w},
gtR(d){var w=this.f
if(w===$){w!==$&&B.ab()
w=this.f=new A.aiF()}return w}}
A.b1a.prototype={
gR_(){return"Termini e Condizioni"},
QU(d){return y.d+d+"."},
gQY(){return y.n},
gQV(){return y.e},
gQZ(){return"Contratto d'uso"},
gQT(){return" e "},
gQW(){return"L'accordo sulla privacy"},
gQX(){return y.k},
gQH(){return[new A.b1f()]},
gO3(){return[new A.b1b(),new A.b1c(),new A.b1d(),new A.b1e()]}}
A.b1o.prototype={
gtI(){return"Impostazioni"},
gQN(d){return"Lingue"},
gtl(){var w=this.d
if(w===$){w!==$&&B.ab()
w=this.d=new A.b1p()}return w},
gSl(){return y.c}}
A.aiD.prototype={
goo(d){return"Consenti l'accesso al tuo album"},
mc(d,e){return y.B+B.l(e)+y.S}}
A.aiF.prototype={
goo(d){return"Consenti l'accesso al tuo archivio"},
mc(d,e){return y.g+B.l(e)+y.j}}
A.b1p.prototype={
gRb(){return"Modalit\xe0 manuale"},
gK0(){return"Modalit\xe0 automatica"},
gFQ(){return"Modalit\xe0 scura"},
gR0(){return"Modalit\xe0 chiara"}}
var z=a.updateTypes(["i({appName!C})"])
A.b1f.prototype={
$1$appName(d){return y.C+B.l(d)+y.Q},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.r}},
$S:4}
A.b1b.prototype={
$1$appName(d){return y.a+B.l(d)+y.l},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.r}},
$S:4}
A.b1c.prototype={
$1$appName(d){return"Per filigranare le tue immagini, "+B.l(d)+y.E},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.r}},
$S:4}
A.b1d.prototype={
$1$appName(d){return"Per poter salvare le immagini, "+B.l(d)+y.u},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.r}},
$S:4}
A.b1e.prototype={
$1$appName(d){return y.C+B.l(d)+y.T},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.r}},
$S:4}
A.b35.prototype={
$1(d){return A.bDk(this.a,d)},
$S:68}
A.b2G.prototype={
$1$flavor(d){switch(d.a){case 0:return"PicGuard"
case 1:return"PicGuard Pro"}},
$C:"$1$flavor",
$R:0,
$D(){return{flavor:C.r}},
$S:165}
A.b2H.prototype={
$2$lastImages$maxImages(d,e){return"Supporta fino a "+B.l(e)+" immagini, puoi anche caricare "+B.l(d)+" immagini."},
$C:"$2$lastImages$maxImages",
$R:0,
$D(){return{lastImages:C.r,maxImages:C.r}},
$S:166}
A.b2I.prototype={
$2$failedNum$succeedNum(d,e){return"Sono state salvate "+B.l(e)+y.I+B.l(d)+" l'immagine(i)."},
$C:"$2$failedNum$succeedNum",
$R:0,
$D(){return{failedNum:C.r,succeedNum:C.r}},
$S:167}
A.b2J.prototype={
$2$buildNumber$version(d,e){return"Versione "+B.l(e)+"("+B.l(d)+")"},
$C:"$2$buildNumber$version",
$R:0,
$D(){return{buildNumber:C.r,version:C.r}},
$S:168}
A.b2K.prototype={
$2$appName$year(d,e){return"Copyright \xa9 "+B.l(e)+" "+B.l(d)+". Tutti i diritti riservati."},
$C:"$2$appName$year",
$R:0,
$D(){return{appName:C.r,year:C.r}},
$S:170}
A.b2M.prototype={
$1$appName(d){return"Di "+B.l(d)},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.r}},
$S:4}
A.b2N.prototype={
$1$appName(d){return"Nascondere "+B.l(d)},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.r}},
$S:4}
A.b2O.prototype={
$1$appName(d){return"Esentato "+B.l(d)},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.r}},
$S:4}
A.b2P.prototype={
$1$appName(d){return y.B+B.l(d)+y.S},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.r}},
$S:4}
A.b2Q.prototype={
$1$appName(d){return y.g+B.l(d)+y.j},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.r}},
$S:4}
A.b2R.prototype={
$1$appName(d){return y.d+B.l(d)+"."},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.r}},
$S:4}
A.b2S.prototype={
$1$appName(d){return y.C+B.l(d)+y.Q},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.r}},
$S:4}
A.b2T.prototype={
$1$appName(d){return y.a+B.l(d)+y.l},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.r}},
$S:4}
A.b2U.prototype={
$1$appName(d){return"Per filigranare le tue immagini, "+B.l(d)+y.E},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.r}},
$S:4}
A.b2V.prototype={
$1$appName(d){return"Per poter salvare le immagini, "+B.l(d)+y.u},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.r}},
$S:4}
A.b2X.prototype={
$1$appName(d){return y.C+B.l(d)+y.T},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.r}},
$S:4};(function installTearOffs(){var w=a.installInstanceTearOff
w(A.aiD.prototype,"gkp",1,0,null,["$1$appName"],["mc"],0,0,0)
w(A.aiF.prototype,"gkp",1,0,null,["$1$appName"],["mc"],0,0,0)})();(function inheritance(){var w=a.inherit,v=a.inheritMany
w(A.a6a,B.fU)
w(A.b1t,B.My)
w(A.b13,B.Mq)
w(A.b17,B.Ms)
w(A.b1v,B.Mz)
w(A.b15,B.Mr)
w(A.b19,B.Mt)
w(A.b1m,B.Mv)
w(A.b1a,B.Mu)
v(B.mx,[A.b1f,A.b1b,A.b1c,A.b1d,A.b1e,A.b35,A.b2G,A.b2H,A.b2I,A.b2J,A.b2K,A.b2M,A.b2N,A.b2O,A.b2P,A.b2Q,A.b2R,A.b2S,A.b2T,A.b2U,A.b2V,A.b2X])
w(A.b1o,B.Mw)
w(A.aiD,B.Bx)
w(A.aiF,B.By)
w(A.b1p,B.Mx)})()
B.b9K(b.typeUniverse,JSON.parse('{"a6a":{"fU":[],"jt":["fc","fU"]}}'))
var y={E:" deve accedere al tuo spazio di archiviazione. (android.permission.READ_EXTERNAL_STORAGE)",u:" deve accedere al tuo spazio di archiviazione. (android.permission.WRITE_EXTERNAL_STORAGE)",Q:" deve accedere alle tue foto. (NSPhotoLibraryUsageDescription)",T:" deve accedere alle tue foto. (android.permission.READ_MEDIA_IMAGES)",l:" deve avere accesso a Internet. (android.permission.INTERNET)",S:" l'autorizzazione a visitare il tuo album.",j:" l'autorizzazione ad accedere al tuo archivio.",I:" l'immagine(i) e non \xe8 stato possibile salvare ",k:", inizia a utilizzare i nostri servizi dopo aver accettato tutti i termini.",p:"ATTENZIONE: Versione di sviluppo, basata automaticamente su ogni commit",g:"Accedi alle Impostazioni del tuo telefono per concedere a ",b:"Alcune lingue non supportano i font personalizzati",n:"Di seguito \xe8 riportato l'elenco delle autorizzazioni richieste da questa APP:",c:"Dopo averlo attivato, la modalit\xe0 scura verr\xe0 attivata o disattivata in base al sistema",_:"Il testo verr\xe0 aggiunto all'immagine come filigrana",d:"La protezione della privacy e delle informazioni personali degli utenti \xe8 un principio fondamentale di ",f:"La spaziatura tra il testo in ogni riga di testo",C:"Per aggiungere una filigrana alle tue immagini, ",a:"Per poter utilizzare il servizio Sentry, ",e:"Prima di utilizzare i servizi di questa APP, ti preghiamo di leggere attentamente e accettare il ",M:"Questa app non caricher\xe0 alcuna immagine sul server, tutte le operazioni vengono completate localmente",N:"Questo certificato \xe8 utilizzato solo per la gestione di xx attivit\xe0 e non \xe8 valido per altri scopi.",B:"Vai alle Impostazioni del tuo telefono per concedere a "}
var x={g:B.ao("i"),c:B.ao("kZ<fc,fU>")}};
((a,b)=>{a[b]=a.current
a.eventLog.push({p:"main.dart.js_1",e:"endPart",h:b})})($__dart_deferred_initializers__,"bV+vokqEQiR2tBiFe9VKn8MKYXY=");