((a,b)=>{a[b]=a[b]||{}})(self,"$__dart_deferred_initializers__")
$__dart_deferred_initializers__.current=function(a,b,c,$){var B,C,A={
bhQ(d,e,f){var w,v,u=null,t=new B.l3(C.q4,x.c),s=new B.l3(C.h5,x.c)
s=new A.a6e(t,s)
s.JZ(d,u,e,u)
w=s.a
v=t.r
v.toString
w.r=v
t.r=A.bCh(s)
return s},
bCd(d,e){switch(e){case"appName":return new A.b3p()
case"homePage.appPreview":return y.p
case"homePage.imageDescription":return"Si prega di caricare le immagini."
case"homePage.description":return y.M
case"homePage.textLabel":return"Testo"
case"homePage.textLabelDescription":return y._
case"homePage.textValidator":return"Inserisci il testo."
case"homePage.textInput":return y.N
case"homePage.colorLabel":return"Colore"
case"homePage.colorValidator":return"Seleziona un colore."
case"homePage.opacityLabel":return"Opacit\xe0"
case"homePage.fontLabel":return"Font"
case"homePage.fontLabelDescription":return y.b
case"homePage.fontValidator":return"Seleziona un font."
case"homePage.fontSizeLabel":return"Dimensione del font"
case"homePage.textGapLabel":return"Spaziatura del testo"
case"homePage.textGapDescription":return y.f
case"homePage.rowGapLabel":return"Interlinea"
case"homePage.rowGapDescription":return"La spaziatura tra ogni riga di testo"
case"homePage.dragging":return"Trascinamento"
case"homePage.cancelDrag":return"Trascinamento annullato"
case"homePage.limitValidator":return new A.b3q()
case"homePage.formatValidator":return"Supportare solo i formati PNG e JPEG"
case"homePage.preview":return"Anteprima"
case"homePage.save":return"Salva"
case"homePage.savedSuccess":return"Salvataggio riuscito"
case"homePage.savedFailure":return"Impossibile salvare l'immagine(i)"
case"homePage.saveInfo":return new A.b3r()
case"aboutPage.version":return new A.b3s()
case"aboutPage.copyright":return new A.b3t()
case"aboutPage.slogan":return"Le tue foto, la tua firma."
case"aboutPage.readme":return"Introduzione del progetto"
case"aboutPage.appLicense":return"Licenza di applicazione"
case"aboutPage.changelog":return"Changelog"
case"aboutPage.thirdPartyLicense":return"Licenze di terze parti"
case"colors.white":return"Bianco"
case"colors.grey":return"Grigio"
case"colors.black":return"Nero"
case"colors.red":return"Rosso"
case"colors.orange":return"Arancione"
case"colors.blue":return"Blu"
case"menus.about":return new A.b3v()
case"menus.hide":return new A.b3w()
case"menus.hideOthers":return"Nascondere gli altri"
case"menus.showAll":return"Mostra tutto"
case"menus.exit":return new A.b3x()
case"menus.help":return"Aiuto"
case"menus.support":return"Supporto"
case"menus.userAgreement":return"Accordo utente"
case"menus.privacy":return"Politica sulla riservatezza"
case"buttons.agree":return"Accetto"
case"buttons.cancel":return"Annulla"
case"buttons.ignore":return"Ignora"
case"buttons.turnOn":return"Accendi"
case"dialogs.selectColor":return"Seleziona un colore"
case"dialogs.selectFont":return"Seleziona un carattere"
case"dialogs.exitConfirm.exit":return"Uscita"
case"dialogs.exitConfirm.cancel":return"Annulla"
case"dialogs.exitConfirm.title":return"Chiudi conferma"
case"dialogs.exitConfirm.description":return"Sei sicuro di voler chiudere questa finestra?"
case"dialogs.permissions.photos.title":return"Consenti l'accesso al tuo album"
case"dialogs.permissions.photos.description":return new A.b3y()
case"dialogs.permissions.storage.title":return"Consenti l'accesso al tuo archivio"
case"dialogs.permissions.storage.description":return new A.b3z()
case"dialogs.licenseDialog.licenseDialogTitle":return"Termini e Condizioni"
case"dialogs.licenseDialog.licenseDialogContentContent":return new A.b3A()
case"dialogs.licenseDialog.licenseDialogContentTip":return y.n
case"dialogs.licenseDialog.licenseDialogContentPrefix":return y.e
case"dialogs.licenseDialog.licenseDialogContentUserAgreement":return"Contratto d'uso"
case"dialogs.licenseDialog.licenseDialogContentAnd":return" e "
case"dialogs.licenseDialog.licenseDialogContentPrivacyPolicy":return"L'accordo sulla privacy"
case"dialogs.licenseDialog.licenseDialogContentSuffix":return y.k
case"dialogs.licenseDialog.iosPermissions.0":return new A.b3B()
case"dialogs.licenseDialog.androidPermissions.0":return new A.b3C()
case"dialogs.licenseDialog.androidPermissions.1":return new A.b3D()
case"dialogs.licenseDialog.androidPermissions.2":return new A.b3E()
case"dialogs.licenseDialog.androidPermissions.3":return new A.b3G()
case"dialogs.settingsDialog.settings":return"Impostazioni"
case"dialogs.settingsDialog.languages":return"Lingue"
case"dialogs.settingsDialog.themes.manual":return"Modalit\xe0 manuale"
case"dialogs.settingsDialog.themes.system":return"Modalit\xe0 automatica"
case"dialogs.settingsDialog.themes.dark":return"Modalit\xe0 scura"
case"dialogs.settingsDialog.themes.light":return"Modalit\xe0 chiara"
case"dialogs.settingsDialog.themesDescription":return y.c
case"locales.en":return"English"
case"locales.it":return"Italian"
case"locales.zh":return"\u4e2d\u6587"
default:return null}},
bCh(d){return new A.b3R(d)},
a6e:function a6e(d,e){var _=this
_.x=d
_.ay=_.ax=_.at=_.as=_.Q=_.z=_.y=$
_.a=e
_.w=_.r=_.f=_.e=_.d=_.c=_.b=$},
b27:function b27(){},
b1I:function b1I(){},
b1M:function b1M(){},
b29:function b29(){},
b1K:function b1K(){},
b1O:function b1O(d,e){var _=this
_.f=d
_.y=_.x=_.w=$
_.a=e
_.e=_.d=_.c=$},
b20:function b20(d,e){var _=this
_.d=d
_.f=_.e=$
_.a=e
_.c=_.b=$},
b1P:function b1P(){},
b1U:function b1U(){},
b1Q:function b1Q(){},
b1R:function b1R(){},
b1S:function b1S(){},
b1T:function b1T(){},
b22:function b22(d,e){var _=this
_.c=d
_.d=$
_.a=e
_.b=$},
aj4:function aj4(){},
aj6:function aj6(){},
b23:function b23(){},
b3R:function b3R(d){this.a=d},
b3p:function b3p(){},
b3q:function b3q(){},
b3r:function b3r(){},
b3s:function b3s(){},
b3t:function b3t(){},
b3v:function b3v(){},
b3w:function b3w(){},
b3x:function b3x(){},
b3y:function b3y(){},
b3z:function b3z(){},
b3A:function b3A(){},
b3B:function b3B(){},
b3C:function b3C(){},
b3D:function b3D(){},
b3E:function b3E(){},
b3G:function b3G(){}}
B=c[0]
C=c[2]
A=a.updateHolder(c[3],A)
A.a6e.prototype={
i(d,e){var w=this.x.wS(e)
return w==null?this.a.wS(e):w},
guy(){var w=this.y
return w===$?this.y=this:w},
lX(d,e){switch(e.a){case 0:return"PicGuard"
case 1:return"PicGuard Pro"}},
geo(){var w=this,v=w.z
if(v===$){w.guy()
w.z!==$&&B.az()
v=w.z=new A.b27()}return v},
gnV(){var w=this,v=w.Q
if(v===$){w.guy()
w.Q!==$&&B.az()
v=w.Q=new A.b1I()}return v},
gpH(){var w=this,v=w.as
if(v===$){w.guy()
w.as!==$&&B.az()
v=w.as=new A.b1M()}return v},
geN(){var w=this,v=w.at
if(v===$){w.guy()
w.at!==$&&B.az()
v=w.at=new A.b29()}return v},
gdT(d){var w=this,v=w.ax
if(v===$){w.guy()
w.ax!==$&&B.az()
v=w.ax=new A.b1K()}return v},
gdM(){var w,v=this,u=v.ay
if(u===$){w=v.guy()
v.ay!==$&&B.az()
u=v.ay=new A.b1O(w,w)}return u},
gR3(){var w=x.g
return B.a6(["en","English","it","Italian","zh","\u4e2d\u6587"],w,w)},
gtC(){return this.x}}
A.b27.prototype={
gO7(){return y.p},
gkq(d){return y.M},
gS8(){return"Testo"},
gS9(){return y._},
gSa(){return"Inserisci il testo."},
gS7(){return y.N},
gOB(){return"Colore"},
gOC(){return"Seleziona un colore."},
gRs(){return"Opacit\xe0"},
gPY(){return"Font"},
gPZ(){return y.b},
gQ0(){return"Seleziona un font."},
gQ_(){return"Dimensione del font"},
gS6(){return"Spaziatura del testo"},
gS5(){return y.f},
gS4(){return"Interlinea"},
gS3(){return"La spaziatura tra ogni riga di testo"},
gPq(){return"Trascinamento"},
QZ(d,e){return"Supporta fino a "+e+" immagini, puoi anche caricare "+d+" immagini."},
gQ2(){return"Supportare solo i formati PNG e JPEG"},
gRI(){return"Anteprima"},
gIY(d){return"Salva"},
gJ0(){return"Salvataggio riuscito"},
gBK(){return"Impossibile salvare l'immagine(i)"},
IZ(d,e){return"Sono state salvate "+e+y.I+d+" l'immagine(i)."}}
A.b1I.prototype={
SE(d,e,f){return"Versione "+f+"("+e+")"},
OX(d,e){return"Copyright \xa9 "+e+" "+d+". Tutti i diritti riservati."},
gJs(){return"Le tue foto, la tua firma."},
gRP(){return"Introduzione del progetto"},
gO6(){return"Licenza di applicazione"},
gOr(){return"Changelog"},
gSe(){return"Licenze di terze parti"}}
A.b1M.prototype={
gSO(){return"Bianco"},
gIW(){return"Grigio"},
gOh(){return"Nero"},
gqk(){return"Rosso"},
gRt(){return"Arancione"},
gpD(){return"Blu"}}
A.b29.prototype={
v0(d){return"Di "+d},
Qt(d){return"Nascondere "+d},
gQu(){return"Nascondere gli altri"},
gJp(){return"Mostra tutto"},
G4(d){return"Esentato "+d},
gAc(){return"Aiuto"},
gu7(){return"Supporto"},
gwB(){return"Accordo utente"},
gwg(){return"Politica sulla riservatezza"}}
A.b1K.prototype={
gO2(){return"Accetto"},
gOk(d){return"Annulla"},
gGP(){return"Ignora"},
gIv(){return"Accendi"}}
A.b1O.prototype={
gJ6(){return"Seleziona un colore"},
gJ7(){return"Seleziona un carattere"},
gnf(d){var w,v=this.w
if(v===$){w=this.f
v=this.w=new A.b20(w,w)}return v},
gln(){var w=this.x
return w===$?this.x=new A.b1P():w},
gk8(){var w,v=this.y
if(v===$){w=this.f
v=this.y=new A.b22(w,w)}return v}}
A.b20.prototype={
gwf(){var w=this.e
return w===$?this.e=new A.aj4():w},
gu_(d){var w=this.f
return w===$?this.f=new A.aj6():w}}
A.b1P.prototype={
gQX(){return"Termini e Condizioni"},
QR(d){return y.d+d+"."},
gQV(){return y.n},
gQS(){return y.e},
gQW(){return"Contratto d'uso"},
gQQ(){return" e "},
gQT(){return"L'accordo sulla privacy"},
gQU(){return y.k},
gQG(){return[new A.b1U()]},
gO3(){return[new A.b1Q(),new A.b1R(),new A.b1S(),new A.b1T()]}}
A.b22.prototype={
gtR(){return"Impostazioni"},
gQL(d){return"Lingue"},
gty(){var w=this.d
return w===$?this.d=new A.b23():w},
gSc(){return y.c}}
A.aj4.prototype={
gfg(d){return"Consenti l'accesso al tuo album"},
m1(d,e){return y.B+B.k(e)+y.S}}
A.aj6.prototype={
gfg(d){return"Consenti l'accesso al tuo archivio"},
m1(d,e){return y.g+B.k(e)+y.j}}
A.b23.prototype={
gR9(){return"Modalit\xe0 manuale"},
gJX(){return"Modalit\xe0 automatica"},
gFJ(){return"Modalit\xe0 scura"},
gQY(){return"Modalit\xe0 chiara"}}
var z=a.updateTypes(["j({appName!F})"])
A.b1U.prototype={
$1$appName(d){return y.C+B.k(d)+y.Q},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.t}},
$S:4}
A.b1Q.prototype={
$1$appName(d){return y.a+B.k(d)+y.l},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.t}},
$S:4}
A.b1R.prototype={
$1$appName(d){return"Per filigranare le tue immagini, "+B.k(d)+y.E},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.t}},
$S:4}
A.b1S.prototype={
$1$appName(d){return"Per poter salvare le immagini, "+B.k(d)+y.u},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.t}},
$S:4}
A.b1T.prototype={
$1$appName(d){return y.C+B.k(d)+y.T},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.t}},
$S:4}
A.b3R.prototype={
$1(d){return A.bCd(this.a,d)},
$S:78}
A.b3p.prototype={
$1$flavor(d){switch(d.a){case 0:return"PicGuard"
case 1:return"PicGuard Pro"}},
$C:"$1$flavor",
$R:0,
$D(){return{flavor:C.t}},
$S:146}
A.b3q.prototype={
$2$lastImages$maxImages(d,e){return"Supporta fino a "+B.k(e)+" immagini, puoi anche caricare "+B.k(d)+" immagini."},
$C:"$2$lastImages$maxImages",
$R:0,
$D(){return{lastImages:C.t,maxImages:C.t}},
$S:145}
A.b3r.prototype={
$2$failedNum$succeedNum(d,e){return"Sono state salvate "+B.k(e)+y.I+B.k(d)+" l'immagine(i)."},
$C:"$2$failedNum$succeedNum",
$R:0,
$D(){return{failedNum:C.t,succeedNum:C.t}},
$S:144}
A.b3s.prototype={
$2$buildNumber$version(d,e){return"Versione "+B.k(e)+"("+B.k(d)+")"},
$C:"$2$buildNumber$version",
$R:0,
$D(){return{buildNumber:C.t,version:C.t}},
$S:143}
A.b3t.prototype={
$2$appName$year(d,e){return"Copyright \xa9 "+B.k(e)+" "+B.k(d)+". Tutti i diritti riservati."},
$C:"$2$appName$year",
$R:0,
$D(){return{appName:C.t,year:C.t}},
$S:142}
A.b3v.prototype={
$1$appName(d){return"Di "+B.k(d)},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.t}},
$S:4}
A.b3w.prototype={
$1$appName(d){return"Nascondere "+B.k(d)},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.t}},
$S:4}
A.b3x.prototype={
$1$appName(d){return"Esentato "+B.k(d)},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.t}},
$S:4}
A.b3y.prototype={
$1$appName(d){return y.B+B.k(d)+y.S},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.t}},
$S:4}
A.b3z.prototype={
$1$appName(d){return y.g+B.k(d)+y.j},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.t}},
$S:4}
A.b3A.prototype={
$1$appName(d){return y.d+B.k(d)+"."},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.t}},
$S:4}
A.b3B.prototype={
$1$appName(d){return y.C+B.k(d)+y.Q},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.t}},
$S:4}
A.b3C.prototype={
$1$appName(d){return y.a+B.k(d)+y.l},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.t}},
$S:4}
A.b3D.prototype={
$1$appName(d){return"Per filigranare le tue immagini, "+B.k(d)+y.E},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.t}},
$S:4}
A.b3E.prototype={
$1$appName(d){return"Per poter salvare le immagini, "+B.k(d)+y.u},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.t}},
$S:4}
A.b3G.prototype={
$1$appName(d){return y.C+B.k(d)+y.T},
$C:"$1$appName",
$R:0,
$D(){return{appName:C.t}},
$S:4};(function installTearOffs(){var w=a.installInstanceTearOff
w(A.aj4.prototype,"gkq",1,0,null,["$1$appName"],["m1"],0,0,0)
w(A.aj6.prototype,"gkq",1,0,null,["$1$appName"],["m1"],0,0,0)})();(function inheritance(){var w=a.inherit,v=a.inheritMany
w(A.a6e,B.fN)
w(A.b27,B.M8)
w(A.b1I,B.M0)
w(A.b1M,B.M2)
w(A.b29,B.M9)
w(A.b1K,B.M1)
w(A.b1O,B.M3)
w(A.b20,B.M5)
w(A.b1P,B.M4)
v(B.mJ,[A.b1U,A.b1Q,A.b1R,A.b1S,A.b1T,A.b3R,A.b3p,A.b3q,A.b3r,A.b3s,A.b3t,A.b3v,A.b3w,A.b3x,A.b3y,A.b3z,A.b3A,A.b3B,A.b3C,A.b3D,A.b3E,A.b3G])
w(A.b22,B.M6)
w(A.aj4,B.Bq)
w(A.aj6,B.Br)
w(A.b23,B.M7)})()
B.b9Y(b.typeUniverse,JSON.parse('{"a6e":{"fN":[],"jv":["fj","fN"]}}'))
var y={E:" deve accedere al tuo spazio di archiviazione. (android.permission.READ_EXTERNAL_STORAGE)",u:" deve accedere al tuo spazio di archiviazione. (android.permission.WRITE_EXTERNAL_STORAGE)",Q:" deve accedere alle tue foto. (NSPhotoLibraryUsageDescription)",T:" deve accedere alle tue foto. (android.permission.READ_MEDIA_IMAGES)",l:" deve avere accesso a Internet. (android.permission.INTERNET)",S:" l'autorizzazione a visitare il tuo album.",j:" l'autorizzazione ad accedere al tuo archivio.",I:" l'immagine(i) e non \xe8 stato possibile salvare ",k:", inizia a utilizzare i nostri servizi dopo aver accettato tutti i termini.",p:"ATTENZIONE: Versione di sviluppo, basata automaticamente su ogni commit",g:"Accedi alle Impostazioni del tuo telefono per concedere a ",b:"Alcune lingue non supportano i font personalizzati",n:"Di seguito \xe8 riportato l'elenco delle autorizzazioni richieste da questa APP:",c:"Dopo averlo attivato, la modalit\xe0 scura verr\xe0 attivata o disattivata in base al sistema",_:"Il testo verr\xe0 aggiunto all'immagine come filigrana",d:"La protezione della privacy e delle informazioni personali degli utenti \xe8 un principio fondamentale di ",f:"La spaziatura tra il testo in ogni riga di testo",C:"Per aggiungere una filigrana alle tue immagini, ",a:"Per poter utilizzare il servizio Sentry, ",e:"Prima di utilizzare i servizi di questa APP, ti preghiamo di leggere attentamente e accettare il ",M:"Questa app non caricher\xe0 alcuna immagine sul server, tutte le operazioni vengono completate localmente",N:"Questo certificato \xe8 utilizzato solo per la gestione di xx attivit\xe0 e non \xe8 valido per altri scopi.",B:"Vai alle Impostazioni del tuo telefono per concedere a "}
var x={g:B.ad("j"),c:B.ad("l3<fj,fN>")}};
(a=>{a["IWSWWPpWmY+tV2JMwgR9y8xfigE="]=a.current})($__dart_deferred_initializers__);